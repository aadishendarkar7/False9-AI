...
try:
    ...
    with live_col2:
        if FOOTBALL_DATA_KEY and st.button("🔄 Refresh", key="refresh_live"):
            get_live_matches.clear()
            st.rerun()

    if not FOOTBALL_DATA_KEY:
        st.warning(
            "Add FOOTBALL_DATA_KEY to .streamlit/secrets.toml (locally) or to "
            "your app's Secrets settings (on Streamlit Cloud) to see live "
            "scores here."
        )
    else:
    ...
    # <script> tags, so this runs through a components.html iframe instead.
    # -----------------------------------------------------------------------
    components.html("""
    <script>
    try {
    ...
    st.error("Something failed on the Dashboard — full details below:")
    st.exception(e)